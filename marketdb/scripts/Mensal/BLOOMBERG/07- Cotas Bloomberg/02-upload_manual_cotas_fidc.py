...
cotas['ano'] = cotas['dt_ref'].str[2]
cotas['mes'] = cotas['dt_ref'].str[1]
cotas['dia'] = cotas['dt_ref'].str[0]
cotas['dt_ref'] = pd.to_datetime(cotas['ano']+cotas['mes']+cotas['dia']).dt.date
del cotas['ano']
del cotas['mes']
del cotas['dia']

cnpj = pd.read_sql_query('SELECT DISTINCT isin, fundo, cnpjfundo_outros, cnpjfundo_1nivel from projeto_inv.xml_quadro_operacoes where produto = "fundo"', connection)
...
cotas = cotas.rename(columns={'Cap. Líquida':'cap_liq', 'Num. Cotistas':'num_cotista', 'pl':'patrimonio', 'Var% em 12 meses':'var_perc_12_meses', 'Var% no ano':'var_perc_ano', 'Var% no dia':'var_perc_dia', 'Var% no mês':'var_perc_mes', 'fundo':'nome_cota', 'isin_fundo':'codigo_isin', 'cnpj_fundo':'cnpj'})
cotas['data_bd'] = datetime.datetime.today()

pd.io.sql.to_sql(cotas, name='fidc_cotas', con=connection,if_exists="append", flavor='mysql', index=0, chunksize=5000)    
...
################################################################################
#
#mes_hoje = datetime.date(2016,6,30)
#hoje = mes_hoje
...
#
#
#cotas['data_bd'] = datetime.date.today()
#
#
#
