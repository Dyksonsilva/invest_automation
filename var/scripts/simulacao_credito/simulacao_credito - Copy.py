...
query='SELECT n.* FROM projeto_inv.curva_ettj_interpol_INNER n INNER JOIN (  SELECT indexador_cod, month(dt_ref ) as mes, year (dt_ref) as ano, max(dt_ref) as max  FROM projeto_inv.curva_ettj_interpol_INNER  GROUP BY 1,2,3) a on a.indexador_cod=n.indexador_cod and a.max=n.dt_ref;'
...
#df_fluxo_sim.to_csv(r"C:\Users\Caio.Deutsch\Desktop\simulacao_reescrita\df_fluxo_sim.csv")

#### ate aqui igual o da Betty

df_fluxo_sim0 = df_fluxo_sim
del df_fluxo_sim['level_0']
del df_fluxo_sim['index']
del df_fluxo_sim['quantidade']
del df_fluxo_sim['cod_rtg']
del df_fluxo_sim['rtg']
del df_fluxo_sim['fv']
del df_fluxo_sim['prazo']
del df_fluxo_sim['tx_spot_sim']

## Exemplo da Localiza para debug...
df_fluxo_sim_localiza = df_fluxo_sim[df_fluxo_sim['contraparte']=='LOCALIZA RENT A CAR SA']

dias_uteis = 22

df_fluxo_sim_localiza['du_ref_times_pv'] = df_fluxo_sim_localiza['du_ref'] * df_fluxo_sim_localiza['pv']


#df_fluxo_sim_localiza['duration'] = pd.to_datetime(df_fluxo_sim_localiza['dt_vencto']) - pd.to_datetime(df_fluxo_sim_localiza['dt_base'])
#df_fluxo_sim_localiza['duration'] = round(df_fluxo_sim_localiza['duration']/np.timedelta64(1,'M'), 0)

df_fluxo_sim_localiza.loc[df_fluxo_sim_localiza['pv'] != 0]
...
df_fluxo_perda=df_fluxo_sim[(pd.to_datetime(df_fluxo_sim['data_ref'])>=pd.to_datetime(df_fluxo_sim['dt_default'])) & (df_fluxo_sim['default']==1)]
df_perda=df_fluxo_perda[['isin', 'tipo_ativo', 'lgd','pv']].copy()
df_perda['pv']=df_perda['pv'].fillna(0)
df_perda_tot=df_perda.groupby(['isin','tipo_ativo', 'lgd'], as_index=False).sum()
df_perda_tot['recuperacao']=np.where(df_perda_tot.tipo_ativo=='DP', 20000000, 0)
df_perda_tot['pv_perda']=(df_perda_tot['pv']*df_perda_tot['lgd']-df_perda_tot['recuperacao']).clip(0,None)
df_perda_tot1=df_perda_tot[['isin', 'pv_perda']].copy()

df_resultado=pd.merge(df_tot, df_perda_tot1, left_on=['isin'], right_on=['isin'], how='left')
df_resultado['pv_perda']=df_resultado['pv_perda'].fillna(0)
df_resultado['perda_perc']=df_resultado['pv_perda']/df_resultado['pv']
...
pd.io.sql.to_sql(df_resultado, name='simulacao_credito', con=connection, if_exists='append', flavor='mysql', index=0)
