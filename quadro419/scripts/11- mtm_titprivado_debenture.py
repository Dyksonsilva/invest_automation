# -*- coding: utf-8 -*-
"""
Created on Fri Apr 15 12:04:59 2016
...
senhabd="root"
root = 'C:/Users/Caio.Deutsch/Documents/Trabalho_EY/Projetos/HDI_automacao_relatorios/materiais_Thiago/codigo_fonte_final/'
dtbase = '20161230'

#Diretório de save de planilhas
save_path = root+'/codigo_fonte_final/Validacoes_outras/'
var_path = 'C:/Users/Caio.Deutsch/Documents/Trabalho_EY/Projetos/HDI_automacao_relatorios/materiais_Thiago/codigo_fonte_final/codigo_fonte_final/feriados_nacionais.csv'
...
x = 'select * from projeto_inv.fluxo_titprivado where data_expiracao>='+'"'+dtbase+'";'

#Importação dos dados
connection=db.connect('localhost', user = 'root', passwd = senhabd, db = 'projeto_inv')
base_fluxo = pd.read_sql(x, con=connection)
connection.close()
...
if len(base_fluxo_deb)!=0:
    base_fluxo_deb = base_fluxo_deb[base_fluxo_deb.dtrel==dtbase].copy()
...
if len(base_fluxo_tit)!=0:
    base_fluxo_tit = base_fluxo_tit[base_fluxo_tit.dtrel==dtbase].copy()   
...
fluxo_fidc = pd.read_excel(end+'fluxo_manual_fidc.xlsx')
...
x = 'select * from projeto_inv.debentures_caracteristicas'

connection=db.connect('localhost', user = 'root', passwd = senhabd, db = 'projeto_inv')

deb_carac = pd.read_sql(x, con=connection)
...
base_fluxo = base_fluxo.sort(['codigo_isin','id_papel','flag','dt_ref','data_bd'],ascending=[True,True,True,True,False])

base_fluxo['dtoperacao'][base_fluxo.dtoperacao<base_fluxo.data_emissao] = base_fluxo['data_emissao'][base_fluxo.dtoperacao<base_fluxo.data_emissao]
...
base=base.sort(['codigo_isin','id_papel','flag', 'dt_ref1','data_bd', 'flag_inclusao'], ascending=[True,True,True,True,False,True])
...
base_mtm['tipo_capitalizacao'] = np.where(base_mtm['tipo_capitalizacao'].isnull(),'Exponencial',base_mtm['tipo_capitalizacao'])
base_mtm['tipo_capitalizacao'] = np.where(base_mtm['tipo_capitalizacao']=='None','Exponencial',base_mtm['tipo_capitalizacao'])

#Preenche com percentual_indexador com 100 quando não está especificado ou está zerado
base_mtm['percentual_indexador'] = np.where(base_mtm['percentual_indexador'].isnull(),100,base_mtm['percentual_indexador'])
base_mtm['percentual_indexador'] = np.where(base_mtm['percentual_indexador']==0,100,base_mtm['percentual_indexador'])

#Divide percentual juros/100 se for >1
base_mtm['percentual_juros'] = np.where(base_mtm['percentual_juros']>1,base_mtm['percentual_juros']/100,base_mtm['percentual_juros'])

#Preenche a data de inicio de rentabilidade do fluxo numeraca
base_mtm['dt_inicio_rentab'][base_mtm.dt_inicio_rentab.isnull()] = base_mtm['data_emissao'][base_mtm.dt_inicio_rentab.isnull()]
...
mtm['dt_inicio'] = np.where(mtm['codigo_isin']==mtm['codigo_isin_lag'], pd.to_datetime(mtm['dt_fim_lag']), pd.to_datetime(mtm['dt_inicio_rentab']))
...
mtm['dt_inicio'] = np.where(mtm['indexador']=='DI1', mtm['dt_inicio'] - pd.DateOffset(months=0, days=1), mtm['dt_inicio'])
...
x = 'select * from projeto_inv.anbima_projecao_indices'
proj = pd.read_sql(x, con=connection)
...
inicio = inicio.rename(columns = {'ano':'ano_inicio', 'mes':'mes_inicio', 'dia':'dia_inicio', 'tx_acum_du': 'tx_acum_du_inicio', 'tx_acum_dc': 'tx_acum_dc_inicio', 'indice_dc': 'indice_dc_inicio', 'indice_du': 'indice_du_inicio'})
fim = fim.rename(columns = {'ano':'ano_fim', 'mes':'mes_fim', 'dia':'dia_fim', 'tx_acum_du': 'tx_acum_du_fim', 'tx_acum_dc': 'tx_acum_dc_fim', 'indice_dc': 'indice_dc_fim', 'indice_du': 'indice_du_fim'})
...
mtm=mtm.sort(['codigo_isin','codigo_cetip','tipo_ativo','data_emissao','data_expiracao','valor_nominal','indexador','percentual_indexador','cod_frequencia_juros','data_primeiro_pagamento_juros','id_bmf_numeraca','data_bd'], ascending=[True, True, True, True,True, True, True, True,True, True, True, True])
mtm=mtm.reset_index(level=None, drop=True, inplace=False, col_level=0, col_fill='')
...
###############################################################################
writer = ExcelWriter(save_path+'erros_mtm_titprivado.xlsx')
...
mtm['fator_index_per'][mtm['indexador_dc_du']==252] = mtm['tx_acum_du_fim'][mtm['indexador_dc_du']==252]/mtm['tx_acum_du_inicio'][mtm['indexador_dc_du']==252]
mtm['fator_index_per'][mtm['indexador_dc_du']==360] = mtm['tx_acum_du_fim'][mtm['indexador_dc_du']==360]/mtm['tx_acum_du_inicio'][mtm['indexador_dc_du']==360]
...
mtm['fator_juros_per'][(mtm['tipo_capitalizacao']=='Exponencial') & (mtm['juros_dc_du']==252)] = mtm['fator_juros_per_du_exp'][(mtm['tipo_capitalizacao']=='Exponencial') & (mtm['juros_dc_du']==252)]
mtm['fator_juros_per'][(mtm['tipo_capitalizacao']=='Exponencial') & (mtm['juros_dc_du']==360)] = mtm['fator_juros_per_dc_exp'][(mtm['tipo_capitalizacao']=='Exponencial') & (mtm['juros_dc_du']==360)]
mtm['fator_juros_per'][(mtm['tipo_capitalizacao']=='Linear') & (mtm['juros_dc_du']==252)] = mtm['fator_juros_per_du_lin'][(mtm['tipo_capitalizacao']=='Linear') & (mtm['juros_dc_du']==252)]
mtm['fator_juros_per'][(mtm['tipo_capitalizacao']=='Linear') & (mtm['juros_dc_du']==360)] = mtm['fator_juros_per_dc_lin'][(mtm['tipo_capitalizacao']=='Linear') & (mtm['juros_dc_du']==360)]
...
mtm_aux['perc_amortizacao'][(mtm_aux.total_amortizacao!=0)&(mtm_aux.perc_amortizacao!=0)] = 1/mtm_aux['num_parc_amt'][(mtm_aux.total_amortizacao!=0)&(mtm_aux.perc_amortizacao!=0)]
...
mtm_aux['perc_amortizacao'] = np.where(mtm_aux['id_papel'].isin(['BRMGIPDBS000']),mtm_aux['perc_amortizacao1']/100,mtm_aux['perc_amortizacao'])
...
cdi_fim = cdi_fim.rename(columns = {'ano':'ano_fim', 'mes':'mes_fim', 'dia':'dia_fim', 'tx_acum_fator_di_inicio':'tx_acum_fator_di_fim'})
...
for i in idx[0:2]:
    ...
    mtm_aux_inicio = mtm_aux_inicio.merge(cdi_inicio, on = ['codigo_isin', 'ano_inicio', 'mes_inicio', 'dia_inicio'],how='left')
    ...
    mtm_aux['tx_acum_fator_di_inicio'][mtm_aux.codigo_isin==mtm_aux['codigo_isin'][i]] = mtm_aux_inicio['tx_acum_fator_di_inicio_aux'][mtm_aux_inicio['tx_acum_fator_di_inicio_aux'].notnull()].copy()
    mtm_aux['tx_acum_fator_di_fim'][mtm_aux.codigo_isin==mtm_aux['codigo_isin'][i]] = mtm_aux_fim['tx_acum_fator_di_fim_aux'][mtm_aux_fim['tx_acum_fator_di_fim_aux'].notnull()].copy()
    del mtm_aux_inicio['tx_acum_fator_di_inicio_aux']
    del mtm_aux_fim['tx_acum_fator_di_fim_aux']


mtm_aux['vne'][mtm_aux['codigo_isin']!=mtm_aux['codigo_isin_lag']] = mtm_aux['valor_nominal'][mtm_aux['codigo_isin']!=mtm_aux['codigo_isin_lag']]
mtm_aux['vne'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']] = mtm_aux['valor_nominal'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']]*(1-mtm_aux['perc_amortizacao_acum_lag'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']])
...
mtm_aux['juros_per'][idx] = mtm_aux['vne'][idx]*(mtm_aux['fator_juros_per'][idx]*mtm_aux['tx_acum_fator_di_fim'][idx]/mtm_aux['tx_acum_fator_di_inicio'][idx]-1)
...
i=1
for i in idx[1:2]:
    mtm_aux['juros_per'][i] = (mtm_aux['vne'][i] + mtm_aux['saldo_dev_juros'][i-1])*(mtm_aux['fator_juros_per'][i]*mtm_aux['tx_acum_fator_di_fim'][i]/mtm_aux['tx_acum_fator_di_inicio'][i]-1) + mtm_aux['saldo_dev_juros'][i-1]
...
mtm_aux['vne'][mtm_aux['codigo_isin']!=mtm_aux['codigo_isin_lag']] = mtm_aux['valor_nominal'][mtm_aux['codigo_isin']!=mtm_aux['codigo_isin_lag']]
mtm_aux['vne'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']] = mtm_aux['valor_nominal'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']]*mtm_aux['fator_index_acum_lag'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']]*(1-mtm_aux['perc_amortizacao_acum_lag'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']])
...
mtm_aux['juros_per'][idx] = mtm_aux['vne'][idx]*mtm_aux['fator_index_per'][idx]*(mtm_aux['fator_juros_per'][idx]-1)
mtm_aux['pagto_juros'][idx] = mtm_aux['juros_per'][idx]*mtm_aux['juros_tipo1'][idx]*mtm_aux['percentual_juros'][idx]
mtm_aux['saldo_dev_juros'][idx] = mtm_aux['juros_per'][idx] - mtm_aux['pagto_juros'][idx]
...
#### Ate aqui ok #####
for i in idx[0:2]:
    mtm_aux['juros_per'][i] = (mtm_aux['vne'][i] + mtm_aux['saldo_dev_juros'][i-1])*mtm_aux['fator_index_per'][i]*(mtm_aux['fator_juros_per'][i]-1) + mtm_aux['saldo_dev_juros'][i-1]
    mtm_aux['pagto_juros'][i] = mtm_aux['juros_per'][i]*mtm_aux['juros_tipo1'][i]*mtm_aux['percentual_juros'][i]
    mtm_aux['saldo_dev_juros'][i] = mtm_aux['juros_per'][i] - mtm_aux['pagto_juros'][i]
...
mtm_aux = mtm_aux.merge(x1,on=['codigo_isin'],how='left')
mtm_aux['perc_amortizacao'][(mtm_aux.total_amortizacao!=0)&(mtm_aux.perc_amortizacao!=0)] = 1/mtm_aux['num_parc_amt'][(mtm_aux.total_amortizacao!=0)&(mtm_aux.perc_amortizacao!=0)]
mtm_aux['perc_amortizacao'] = np.where(mtm_aux['flag'].isin(['FM1']),mtm_aux['perc_amortizacao1'],mtm_aux['perc_amortizacao'])
...
mtm_aux['perc_amortizacao_atualizado'][mtm_aux.codigo_isin==mtm_aux.codigo_isin_lag] = mtm_aux['perc_amortizacao'][mtm_aux.codigo_isin==mtm_aux.codigo_isin_lag]/(1-mtm_aux['perc_amortizacao_acum_lag'][mtm_aux.codigo_isin==mtm_aux.codigo_isin_lag])

mtm_aux['perc_amortizacao_atualizado'][mtm_aux.codigo_isin_final!=mtm_aux.codigo_isin] = 1
mtm_aux['fator_amortizacao_acum_dt'] = 1 - mtm_aux['perc_amortizacao_atualizado']
mtm_aux['fator_amortizacao_acum_dt'] = mtm_aux[['codigo_isin','fator_amortizacao_acum_dt']].groupby(['codigo_isin']).agg(['cumprod'])
...
mtm_aux['vne'][mtm_aux['codigo_isin']!=mtm_aux['codigo_isin_lag']] = mtm_aux['valor_nominal'][mtm_aux['codigo_isin']!=mtm_aux['codigo_isin_lag']]
mtm_aux['vne'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']] = mtm_aux['valor_nominal'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']]*mtm_aux['fator_index_acum_lag'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']]*mtm_aux['fator_amortizacao_acum_dt_lag'][mtm_aux['codigo_isin']==mtm_aux['codigo_isin_lag']]
mtm_aux['vna'] = mtm_aux['vne']*mtm_aux['fator_index_per']
mtm_aux['vl_amortizacao'] = mtm_aux['vna']*mtm_aux['perc_amortizacao_atualizado']


aux = mtm_aux[['tipo','dt_fim','data_primeiro_pagamento_juros','perc_amortizacao_acum_lag','perc_amortizacao_atualizado']].copy()
...
print('11',time.time() - start_time)
###############################################################################
#12 - Criação das ETTJS
###############################################################################

print('12',time.time() - start_time)
...
mtm_dia = inicio.copy()
mtm_dia = mtm_dia.rename(columns = {'ano_inicio':'ano_mtm', 'mes_inicio':'mes_mtm', 'dia_inicio':'dia_mtm', 'indice_dc_inicio': 'indice_dc_mtm','indice_du_inicio': 'indice_du_mtm' })
...
dt_ref_dia = dt_ref_dia.rename(columns = {'ano_mtm':'ano_dt_ref2', 'mes_mtm':'mes_dt_ref2', 'dia_mtm':'dia_dt_ref2', 'indice_dc_mtm':'indice_dc_dt_ref2','indice_du_mtm':'indice_du_dt_ref2' })
...
###############################################################################
connection=db.connect('localhost', user = 'root', passwd = senhabd, db = 'projeto_inv')
x='select * from projeto_inv.curva_ettj_interpol_2016_12 where indexador_cod in("PRE","DIM","DIC");'
...
ettj['indexador']=np.where(ettj['indexador_cod']=='DIC','IPCA',np.where(ettj['indexador_cod']=='DIM', 'IGP', 'PRE'))
...
tp_mtm['duration'] = np.where(tp_mtm['fluxo_positivo_sum']!=0,tp_mtm['fluxo_ponderado_sum']/tp_mtm['fluxo_positivo_sum'],0)
...
print('14',time.time() - start_time)
###############################################################################
#15 - Impressão
###############################################################################

print('15',time.time() - start_time)
...
tp_mtm.to_excel(save_path+'/mtm_titprivado_debentures.xlsx')

tp_mtm['data_bd'] = datetime.datetime.today()

##Conexão com Banco de Dados
connection=db.connect('localhost', user = 'root', passwd = senhabd, db = 'projeto_inv')
pd.io.sql.to_sql(tp_mtm, name='mtm_titprivado_thiago', con=connection, if_exists='append', flavor='mysql', index=0)
connection.close()
