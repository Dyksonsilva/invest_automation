def titpublico_final_pv_ettj():
    ...
    #Informações XML
    connection = db.connect('localhost', user = 'root', passwd = 'rott', db = 'projeto_inv', charset='utf8')
    ...
    #Mudança de formatos
    #fluxo['dtvencimento'] = fluxo['dtvencimento'].astype(str)
    #fluxo['dtvencimento'] = fluxo['dtvencimento'].str.replace('-','')
    #fluxo['codativo'] = fluxo['codativo'].astype(str)
    #fluxo['dt_ref'] = fluxo['dt_ref'].dt.date
    ...
    #Mudança de formato
    #vna['codativo'] = vna['codativo'].astype(str)
    ...
    dt_base = str(dt_base)
    query = 'SELECT * FROM projeto_inv.curva_ettj_interpol_' + dtbase[0] + '_' + dtbase[1] + ' WHERE indexador_cod in("PRE","DIM","DIC","SLP");'
