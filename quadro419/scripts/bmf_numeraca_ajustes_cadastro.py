...
#base_dir = root + '/codigo_fonte_final/'
#end_val = root + '/codigo_fonte_final/Validacoes/'
#xlsx_path = end_val + 'controle_titprivado.xlsx'
...
query = 'SELECT * FROM projeto_inv.bmf_numeraca'
...
x = 'select * from projeto_inv.xml_titprivado_org '

titprivado_xml = pd.read_sql(x, con=connection)
...
titprivado_xml = titprivado_xml[titprivado_xml.dtrel == dtbase].copy()
titprivado_xml = titprivado_xml[titprivado_xml.data_bd == max(titprivado_xml.data_bd)]

x = 'select * from projeto_inv.xml_header_org '

header_xml = pd.read_sql(x, con=connection)

# Seleciona última carga com dtposicao = data_base
header_xml = header_xml[
    header_xml.dtposicao == datetime.date(int(dtbase[0:4]), int(dtbase[4:6]), int(dtbase[6:8]))].copy()
...
###############################################################################
dtbase = datetime.date(int(dtbase[0:4]), int(dtbase[4:6]), int(dtbase[6:8]))

idx = titprivado[(titprivado.flag.isnull()) & (titprivado.dtvencimento >= dtbase)].index.tolist()
...
manual = pd.read_excel(base_dir + '/caracteristicas.xlsx')
...
isin_list = pd.read_excel(base_dir + '/bullet_12meses.xlsx')
...
# Lista de papéis na transformação: JUROS '' bullet -> 'A' anual
isin_list = pd.read_excel(base_dir + '/12meses_bullet.xlsx')
titprivado = titprivado.merge(isin_list, on=['codigo_isin'], how='left')
